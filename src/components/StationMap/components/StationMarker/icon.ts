...
export function getFillColor({bikes}: StationNode): string {
  if (bikes > 10) {
    return 'forestgreen';
  }
...
function markerWithFillColor(station: StationNode) {
...
export function createIcon(station: StationNode): google.maps.Icon {
  return {
